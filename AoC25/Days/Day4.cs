...
public class Day4 : IDay
{
    ...
    public string Part2(string input)
    {
        ...
        while (true)
        {
            var rollsRemoved = CountRolls(map, true);
            totalRolls += rollsRemoved;
    ...
    private int CountRolls(List<List<bool>> map, bool removeRolls = false)
    {
        var totalRolls = 0;
        ...
        for (var line = 0; line < map.Count; line++)
        {
            for (var column = 0; column < map[line].Count; column++)
            {
                ...
                foreach (var neighbourOffset in neighbourOffsets)
                {
                    ...
                    if (neighbourY >= 0 && neighbourY <= map.Count - 1 && neighbourX >= 0 &&
                        neighbourX <= map[line].Count - 1)
                    {
